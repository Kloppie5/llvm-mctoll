...
class ARMArgumentRaiser : public RaiserPass {

public:
  ARMArgumentRaiser(ModuleRaiser &MR) : RaiserPass(MR) {};
...
private:
  ...
  /// MachineInstr.
  void updateParameterInstr(MachineFunction *MF, Function *F);
  /// Move arguments which are passed by ARM registers(R0 - R3) from function
  /// arg.x to corresponding registers in entry block.
  void moveArgumentToRegister(unsigned Reg, MachineFunction *MF, MachineBasicBlock &PMBB);
