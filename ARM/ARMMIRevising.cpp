...
public:
  ...
  ARMMIRevising(ModuleRaiser &MR, MCInstRaiser *MCIR)
      : RaiserPass(MR, "ARMMIRevising"), MCIR(MCIR) {}
    ...
    for (section_iterator SecIter : Elf32LEObjFile->sections()) {
      ...
      if ((SecStart <= PLTEndOff) && (SecEnd >= PLTEndOff)) {
        ...
        ArrayRef<uint8_t> Bytes(
            reinterpret_cast<const uint8_t *>(SecData.data()), SecData.size());
  ...
  void relocateBranch(MachineInstr &MInst) {
    ...
    if ((EType == ELF::ET_DYN) || (EType == ELF::ET_EXEC)) {
      ...
      if (reloc == nullptr) {
        ...
        LLVM_DEBUG(dbgs() << "No relocation for instruction at offset "
                          << Offset << "\n");
        return;
      }
  ...
  const Value *getGlobalValueByOffset(int64_t MCInstOffset, uint64_t PCOffset) {
    ...
    LLVMContext &LCTX = MR.M->getContext();
    if (Symbol != nullptr) {
      ...
      GlobVal = MR.M->getGlobalVariable(SymName);
      if (GlobVal == nullptr) {
        ...
        if (!SymbOrErr)
          ...
          auto GlobVar =
              new GlobalVariable(*MR.M, GlobValTy, false /* isConstant */, Linkage,
                                 GlobInit, SymName);
          uint64_t Align = 32;
          switch (SymSz) {
          default:
          case 4:
            // When the symbol size is bigger than 4 bytes, identify the object
            // as array or struct and set alignment to 32 bits.
            Align = 32;
            break;
          case 2:
            Align = 16;
      ...
      if (GlobVal == nullptr) {
        ...
        if (MCIR->getMCInstAt(Index) != MCIR->const_mcinstr_end()) {
          ...
          GlobVal = MR.M->getGlobalVariable(LocalNameRef);
          if (GlobVal == nullptr) {
              ...
              if ((SecStart <= DataAddr) && (SecEnd >= DataAddr)) {
                if (SecIter->isData()) {
                  ...
                  auto GlobalStrConstVal = new GlobalVariable(
                      *MR.M, StrConstant->getType(), /* isConstant */ true,
            ...
            if (GlobVal == nullptr) {
              ...
              auto GlobVar = new GlobalVariable(*MR.M, ty, /* isConstant */ true,
  ...
  bool reviseMI(MachineInstr &MInst) {
  ...
  bool revise(MachineFunction &MF) {
    bool changed = false;
